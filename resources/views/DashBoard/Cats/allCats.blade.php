...
@section('body')
    <div class="container">
        ...
        <div class="table-responsive">
            <div class="table-wrapper">
                <div class="table-title ">
                    <h2 class="text-light ">Manage <b>Categorys</b></h2>
                </div>
                <form class="d-flex justify-content-center" action="{{url("/admin/catsearch")}}" method="GET">
        ...
        <form class="mb-3 d-flex justify-content-between" method="POST" action="{{url("/admin/addcat")}}">
            @csrf
        ...
        <table class="table table-striped table-hover border text-center">
            ...
            <tbody>
                ...
                <tr>
                    <td class="w-75">
                        <div id="case1"> {{$cat->name}} </div>
                        <form method="POST" id="case2" style="display: none" action="{{url("/admin/editcat/$cat->id")}}">
                            @csrf
                            <input class=" w-25" type="text" name="name" placeholder="Category new name" value="{{$cat->name}}">
                            <input class="btn btn-success " type="submit" value="Update">
                        </form>
                    </td>
                    <td class="w-25">
                        <i id="toggle" class="fas fa-edit text-warning" style="cursor: pointer"></i>
                        <a href="{{url("admin/deletecat/$cat->id")}}" class="delete" ><i class="fas fa-trash-alt"></i></a>
                    </td>
                </tr>
                @endforeach
